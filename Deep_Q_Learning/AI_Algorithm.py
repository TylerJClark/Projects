...
#add watch while learning
#fix cuda
#add human vs AI
#save network
#fix why the AI isnt learning anything
#try a different game? pole cart? Flappy bird?
...
def trainGame1v1(game,hiddenLayers,maxEpisodes,maxSteps,
    ...
    for episode in range(maxEpisodes):
        ...
        if not watch:
            ...
            while watchDone and manager.getAdvanceMove():
                time.sleep(0.5)
            manager.reset()
        ...
        turn = 0 
        storedStateP0 = deepcopy(currentState)
        #print(storedStateP0)
        storedStateP1 = deepcopy(currentState)
        lastStateP0 = deepcopy(currentState)
        lastStateP1 = deepcopy(currentState)
        ...
        for step in range(maxSteps):
            ...
            """if turn == 0: #changes turn
                if storedActionP0 != None:
                    ...
                    #print(memory)
                    print("Pushed Memory:\nstate: Player0 turn\n")
                    for i in lastStateP0:
                        print(i)

                    print("storedStateP0\n")
                    for i in storedStateP0:
                        print(i)

                    print("",storedActionP0)
            else:
            ...
            if watch:
                chosenAction,probabilities = myAgent.select_action(currentState,policyNetwork,posActions,0)
                for i in range(len(probabilities)):
                    probabilities[i] = "{0:.2f}".format(round(probabilities[i],2))
                    
                manager.sendProb(probabilities)
            else:
                chosenAction,probabilites = myAgent.select_action(currentState,policyNetwork,posActions,-1)
            ...
            currentState,reward,done,posActions,lastState = manager.take_action(chosenAction)
            #rewards is boolean WHY
            #print("reward",reward)
            #print("done",done)
            
            if turn == 0:
                if not done and storedActionP1 != None:
                    x = Experience(torch.tensor(storedStateP1,device = device).reshape(1,-1), torch.tensor([storedActionP1],device = device),
                                torch.tensor(currentState,device = device).reshape(1,-1), torch.tensor([storedRewardP1],device = device))

                    if step > 5:
                        memory.push(x)

                    elif not memory.check_copy(x):
                        memory.push(x)
                        

                    """print("Turn 1 pushes:")
                    print("currentState:\n ")
                    for i in storedStateP1:
                        print(i)

                    print("storedAction: ",storedActionP1)
                    print("nextState:\n")
                    for i in currentState:
                        print(i)
                    print("Reward:",storedRewardP1)"""
                if not done:
                    storedStateP1 = deepcopy(currentState)
                lastStateP0 = deepcopy(lastState)
                #print(id(storedStateP0))
                #print(id(currentState))
                storedRewardP0 = reward
                turn = 1
                

            else:
                if not done and storedActionP0 != None:
                    x = Experience(torch.tensor(storedStateP0,device = device).reshape(1,-1), torch.tensor([storedActionP0],device = device),
                                               torch.tensor(currentState,device = device).reshape(1,-1), torch.tensor([storedRewardP0],device = device))
                    ...
                    print("storedAction: ",storedActionP0)
                    print("nextState:\n")
                    for i in currentState:
                        print(i)
                    print("Reward:",storedRewardP0)"""
                if not done:
                    storedStateP0 = deepcopy(currentState)
                lastStateP1 = deepcopy(lastState)
            ...
            if done:
                ...
                """
                if turn == 0:
                    memory.push(Experience(torch.tensor(storedStateP1,device = device).reshape(1,-1), torch.tensor([storedActionP1],device = device), torch.tensor(torch.tensor(currentState),device = device).reshape(1,-1), torch.tensor(torch.tensor([1.]),device = device)))
                    memory.push(Experience(torch.tensor(storedStateP0,device = device).reshape(1,-1), torch.tensor([storedActionP0],device = device), torch.tensor(torch.tensor(manager.getFinalState()).reshape(1,-1),device = device), torch.tensor(torch.tensor([-1.],device = device))))
                    print("state: \n")
                    for i in storedStateP1:
                        print(i)
                    print("player1 wins,player1 memory\nreward\n",reward)

                    print("state: \n")
                    for i in storedStateP0:
                        print(i)
                    print("player1 wins,player0 memory\nreward\n",manager.getReward())
                    
                    #print("player1 wins,player0 memory\n",storedStateP0,"reward\n",manager.getFinalReward())
                    
                else:#torch.tensor(state).reshape(1,-1)
                    memory.push(Experience(torch.tensor(storedStateP1,device = device).reshape(1,-1), torch.tensor([storedActionP1],device = device), torch.tensor(manager.getFinalState(),device = device).reshape(1,-1), torch.tensor([-1.],device = device)))
                    memory.push(Experience(torch.tensor(storedStateP0,device = device).reshape(1,-1), torch.tensor([storedActionP0],device = device), torch.tensor(currentState,device = device).reshape(1,-1), torch.tensor([1.],device = device)))
                    #print("player0 wins,player1 memory\n",storedStateP1,"reward\n",manager.getFinalReward())
                    #print("player0 wins,player0 memory\n",storedStateP0,"reward\n",reward)
                    print("state: \n")
                    for i in storedStateP0:
                        print(i)
                    print("player0 wins,player0 memory\nreward\n",reward)
                ...
                if turn == 0:
                    x = Experience(torch.tensor(storedStateP1,device = device).reshape(1,-1), torch.tensor([storedActionP1],device = device), torch.tensor(torch.tensor(currentState),device = device).reshape(1,-1), torch.tensor(torch.tensor([reward]),device = device))
                    memory.push(x)

                    x = Experience(torch.tensor(storedStateP0,device = device).reshape(1,-1), torch.tensor([storedActionP0],device = device), torch.tensor(torch.tensor(manager.getFinalState()).reshape(1,-1),device = device), torch.tensor([manager.getFinalReward()],device = device))
                    memory.push(x)
                    #print("player1 wins,player1 memory\n",storedStateP1,"reward\n",reward)
                    #print("player1 wins,player0 memory\n",storedStateP0,"reward\n",manager.getFinalReward())
                    """print("final push, player 1, player 1 wins")
                    print("current State\n")
                    for i in storedStateP1:
                        print(i)
                    print("Action",storedActionP1)
                    print("next State",currentState)
                    print("reward",reward)

                    print("final push, player 0, player 1 wins")
                    print("current State\n")
                    for i in storedStateP0:
                        print(i)
                    print("Action",storedActionP0)
                    print("next State",manager.getFinalState())
                    print("reward",manager.getFinalReward())"""
                    


                    
                else:#torch.tensor(state).reshape(1,-1)
                    x = Experience(torch.tensor(storedStateP1,device = device).reshape(1,-1), torch.tensor([storedActionP1],device = device), torch.tensor(manager.getFinalState(),device = device).reshape(1,-1), torch.tensor([manager.getFinalReward()],device = device))
                    memory.push(x)

                    x = Experience(torch.tensor(storedStateP0,device = device).reshape(1,-1), torch.tensor([storedActionP0],device = device), torch.tensor(currentState,device = device).reshape(1,-1), torch.tensor([reward],device = device))
                    memory.push(x)
                    #print("player0 wins,player1 memory\n",storedStateP1,"reward\n",manager.getFinalReward())
                    #print("player0 wins,player0 memory\n",storedStateP0,"reward\n",reward)

                    """print("final push, player 1, player 0 wins")
                    print("current State\n")
                    for i in storedStateP1:
                        print(i)
                    print("Action",storedActionP1)
                    ...
                    print("final push, player 0, player 0 wins")
                    print("current State\n")
                    for i in storedStateP0:
                        print(i)
                    print("Action",storedActionP0)
            ...
            if memory.can_provide_sample(batchSize):
                ...
                target_q_values = tanh((next_q_values * discountRate) + rewards)
                
                #print("current",current_q_values)
                """if episode % 100 == 0:
                    print("current",current_q_values)
                    print("target",target_q_values)
                    print("next q value",next_q_values)
        ...
        if memory.can_provide_sample(batchSize) and episode % 50 == 0:
            print("Explore probability",myAgent.rate)
            #print("current",current_q_values)
            #print("target",target_q_values)
            #print("layers",policyNetwork.fc1.weight)
...
def main(preset):
    ...
    if preset == False:
        gamesPlayedButton = Button(990,570,200,70,grey,blue,True,(0,0,0),text = "Games Played: 0")
    ...
    ddiscountRate = 0.99
    dexploreDecay = 0.0001
    dbatchSize = 256
    
    
    maxEpisodeIncrease2 = Button(165,410,70,70,grey,blue,True,(0,0,0),text = ">")
    maxEpisodesButton2 = Button(50,490,300,70,grey,blue,True,(0,0,0),text = "Max Episodes:" + str(maxEpisodes))
    maxEpisodeDecrease2 = Button(165,570,70,70,grey,blue,True,(0,0,0),text = "<")
    ...
    while running:
        ...
        if position == "home":
            ...
            gamesPlayedButton = Button(950,570,300,70,grey,blue,True,(0,0,0),text = "Games Played:" + str(gamesPlayed))

            
            gamesPlayedButton.create(screen)
            maxEpisodesButton2.create(screen)
            maxEpisodeIncrease2.create(screen)
            maxEpisodeDecrease2.create(screen)
            maxEpisodeIncrease2.hovering(pos)
            maxEpisodeDecrease2.hovering(pos)

            if maxEpisodeIncrease2.click(pos,mouseUp):
                maxEpisodes += 50
                maxEpisodesButton2 = Button(50,490,300,70,grey,blue,True,(0,0,0),text = "Max Episodes:" + str(maxEpisodes))
                maxEpisodesButton = Button(490,255,300,50,grey,blue,True,(0,0,0),text = "Max Episodes:" + str(maxEpisodes))


            if maxEpisodeDecrease2.click(pos,mouseUp):
                maxEpisodes -= 50
                maxEpisodesButton2 = Button(50,490,300,70,grey,blue,True,(0,0,0),text = "Max Episodes:" + str(maxEpisodes))
                maxEpisodesButton = Button(490,255,300,50,grey,blue,True,(0,0,0),text = "Max Episodes:" + str(maxEpisodes))
